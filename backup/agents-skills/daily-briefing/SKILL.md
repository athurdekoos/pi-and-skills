...
### Waiting On ([N] items, [X] days avg)
- [ ] **[Item]** — waiting on [person] since [date]

### ✅ Done This Week: [N] tasks
...
*Say `todo [task]` anytime to add to your list*
```
...
**Always (no connectors needed):**
```
0. Todo List → Read ~/TASKS.md
   - Pull Active tasks (highlight any with due dates today/overdue)
   - Pull Waiting On items (flag anything waiting 3+ days)
   - Count: active tasks, waiting items, completed this week
```
...
**Todos:** [N] active | [N] overdue — [top 3 task titles]
...
- [ ] [Waiting on items flagged]

*Any new todos captured today have been added to ~/TASKS.md*
```
...
- **todo** — Quick-capture tasks to ~/TASKS.md (say "todo [task]" anytime)
- **task-management** — Full task management with sections and dashboard
